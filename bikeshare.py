...
month_names, day_names = list(map(lambda month:month.lower(),calendar.month_name)), list(map(lambda day:day.lower(), calendar.day_name))
month_names[0] = "all"
...
def get_filters() -> tuple:
    ...
    while city is None or city not in CITY_DATA.keys():
        city = input("Enter city name to filter by (chicago/new york city/washington):")
        city = city.lower()
        if not city or city not in CITY_DATA.keys():
            print("Enter a valid city name as input ({}).".format(", ".join(CITY_DATA.keys())))
    ...
    while month is None or month not in month_names:
        ...
        if not month or month not in month_names:
            print("Enter a valid month name as input ({}).".format(", ".join(month_names)))   
...
def time_stats(df: pd.DataFrame):
    ...
    print("The most common rental month: {}".format(month_names[df['month'].mode()[0]]))
    
    prompt_start_time = time.time()
    graph_prompt = input("Generate graph of rentals by month? (y[es]/s[ave]/n[o]): ")
    ...
    print("The most common rental day of the week: {}".format(df['day_of_week'].mode()[0]))
    
    prompt_start_time += time.time()
    graph_prompt = input("Generate graph of rentals by day of week? (y[es]/s[ave]/n[o]): ")
    prompt_end_time += time.time()
    if graph_prompt.lower() in ['y', 'yes', 's', 'save']:
    ...
    prompt_start_time += time.time()
    graph_prompt = input("Generate graph of rentals by hour [warning - messy]? (y[es]/s[ave]/n[o]): ")
    prompt_end_time += time.time()
    if graph_prompt in ['y', 'yes', 's', 'save']:
        
        create_data_graph(df, ['hour'], ['hour'], graph_title="Rides by Hour",
                 graph_type='bar', xticks=None, xtick_labels=None, yticks=None, ytick_labels=None,
                 xlabel='Rides Booked', ylabel='Hour', color_theme='matrix', horizontal=True, saveplot=graph_prompt)
        prompt_start_time += time.time()
        input("\nPress any key to continue...")
        prompt_end_time += time.time()
    
    
    print("\nThis took %s seconds." % (time.time() - start_time - (prompt_end_time - prompt_start_time)))
    print('-'*40)
...
def station_stats(df: pd.DataFrame):
    ...
    print("\n The most common start-end station combinations:{}".format((df["Start Station"] + "-" + df["End Station"]).mode()[0]))
...
def trip_duration_stats(df: pd.DataFrame):
    ...
    print("The mean travel time for a trip is {} seconds.".format(df['Trip Duration'].mean()))
...
def user_stats(df: pd.DataFrame, city:str):
    ...
    if city == 'washington':
        ...
        print("\nThe earliest birth year of a user is: {:.0f}".format(df['Birth Year'].dropna().sort_values(ascending=True).head(1).item()))
        print("The most recent birth year of a user is: {:.0f}".format(df['Birth Year'].dropna().sort_values(ascending=False).head(1).item()))
        print("The most common birth year of a user is: {:.0f}".format(df['Birth Year'].mode()[0]))

    # Display the average trip time, by subscription type
    print("\nThe average trip duration (HH:MM:SS), by customer type is:")
    print(pd.to_timedelta(df.groupby(["User Type"])["Trip Duration"].mean().round(), unit='s').to_string())
    print("\nThis took %s seconds." % (time.time() - start_time))
    print('-'*40)
...
def display_raw_data(df: pd.DataFrame, start_pos: int, batch_size: int = 5):
    ...
    json_data = df.iloc[start_pos:start_pos + batch_size,:].to_dict(orient='records')
    pprint(json_data)
